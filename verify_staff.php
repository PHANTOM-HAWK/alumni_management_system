...
<div class="container">
  ...
  if(mysqli_num_rows($res) > 0)
  {
	  while($row = mysqli_fetch_array($res))
	  {
		echo "<tr>
		<td>".$c++."</td>
		<td><img src='upload/staff/".$row[9]."' width='100px' height='100px' alt='$row[1]'/></td>
		<td>".$row[1]."</td>
		<td>".$row[2]."</td>
		<td>".$row[3]."</td>
		<td>".$row[4]."</td>
		...
		<td><a href='verify_staff.php?id=$row[0]' >Approve</a>&nbsp;/&nbsp;<a href='verify_staff.php?did=$row[0]'>Reject</a></td>
	  </tr>";
	  }
   }
  else	  
