...
<div class="container">
  ...
  <?php
  $res = mysqli_query($con, "Select * from tbluser inner join tblcourse on tbluser.courseid=tblcourse.courseid inner join states on tbluser.location=states.locid where  tbluser.status='Inactive'");
  $c = 1;
  if(mysqli_num_rows($res) > 0)
  {
	  while($row = mysqli_fetch_array($res))
	   {  echo "<tr>
		...
		<td>".$row['mfee']."</td>
		<td>".$row['paytype']."</td>
		<td><a href='verify_reg.php?id=$row[0]' >Approve</a>&nbsp;/&nbsp;<a href='verify_reg.php?did=$row[0]'>Reject</a></td>
		</tr>";
	   }
  }
  else	  
