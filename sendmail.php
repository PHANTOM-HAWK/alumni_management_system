...
function sendmail($toaddress,$subject,$message,$name)
{
	...
	$mail->Username = 'infobca@dentaldiary.in';                 // SMTP username
	$mail->Password = 'q1w2e3r4';                           // SMTP password
	$mail->SMTPSecure = 'ssl';                            // Enable TLS encryption, `ssl` also accepted
	$mail->Port = 465;                                    // TCP port to connect to
	
	$mail->From = 'infobca@dentaldiary.in';
	$mail->FromName = 'ALUMNEE MANAGEMENT SYSTEM';
	$mail->addAddress($toaddress, $name);     // Add a recipient
	$mail->addAddress($toaddress);               // Name is optional
	$mail->addReplyTo($toaddress, 'ALUMNEE MANAGEMENT SYSTEM');
	$mail->addCC('cc@example.com');
	$mail->addBCC('divya.technopulse@gmail.com');
